...
template <typename T> std::string getOrdinalSuffix(T n) {
...
/// More efficient than libc, because doesn't respect locale. But for some
/// functions table implementation could be better.
...
/// Since |isWhiteSpaceASCII()| is used inside algorithms it's easier to
/// implement another function than add extra argument.
...
inline bool isPunctuationASCII(char c) {
  ...
  return (uc >= 33 && uc <= 47) || (uc >= 58 && uc <= 64) ||
         (uc >= 91 && uc <= 96) || (uc >= 123 && uc <= 125);
}

inline bool isNumberSeparator(bool is_start_of_block, bool is_hex,
...
inline const char *skipWhitespacesUTF8(const char *pos, const char *end) {
  ...
  /// Code points: 0085 00A0 180E 2000..200A 2028..2029 200B..200D 202F 205F
  /// 2060 3000 FEFF The corresponding UTF-8 is: C285 C2A0 E1A08E E28080..E2808A
  /// E280A8..E280A9 E2808B..E2808D E280AF E2819F E281A0 E38080 EFBBBF
...
template <typename F> std::string trim(const std::string &str, F &&predicate) {
  size_t cut_front = 0;
  size_t cut_back = 0;
...
inline bool isValidIdentifier(std::string_view str) {
  return !str.empty() && isValidIdentifierBegin(str[0]) &&
         std::all_of(str.begin() + 1, str.end(), isWordCharASCII)
         /// NULL is not a valid identifier in SQL, any case.
         &&
         !(str.size() == strlen("null") && toLowerIfAlphaASCII(str[0]) == 'n' &&
