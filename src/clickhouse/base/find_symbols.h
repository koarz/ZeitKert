 ...
 * It is similar to 'strpbrk', 'strcspn' (and 'strchr', 'memchr' in the case of
 * one symbol and '\0'), but with the following differences:
 ...
 *
 * Uses SSE 2 in case of small number of symbols for search and SSE 4.2 in the
 * case of large number of symbols, that have more than 2x performance advantage
 * over trivial loop in the case of parsing tab-separated dump with (probably
 * escaped) string fields. In the case of parsing tab separated dump with short
 * strings, there is no performance degradation over trivial loop.
 *
 * Note: the optimal threshold to choose between SSE 2 and SSE 4.2 may depend on
 * CPU model.
 *
...
struct SearchSymbols {
  ...
  explicit SearchSymbols(std::string in) : str(std::move(in)) {
    ...
    simd_vector =
        _mm_loadu_si128(reinterpret_cast<const __m128i *>(tmp_safety_buffer));
#endif
  }
...
namespace detail {
template <char... chars> constexpr bool is_in(char x) {
  return ((x == chars) || ...);
...
#if defined(__SSE2__)
template <char s0> inline __m128i mm_is_in(__m128i bytes) {
...
inline std::array<__m128i, 16u> mm_is_in_prepare(const char *symbols,
                                                 size_t num_chars) {
...
inline __m128i mm_is_in_execute(__m128i bytes,
                                const std::array<__m128i, 16u> &needles) {
  __m128i accumulator = _mm_setzero_si128();
...
template <bool positive> constexpr bool maybe_negate(bool x) {
  return x == positive;
}

template <bool positive> constexpr uint16_t maybe_negate(uint16_t x) {
...
inline const char *find_first_symbols_sse2(const char *const begin,
...
inline const char *
find_first_symbols_sse2(const char *const begin, const char *const end,
...
template <bool positive, ReturnMode return_mode, char... symbols>
inline const char *find_last_symbols_sse2(const char *const begin,
                                          const char *const end) {
  const char *pos = end;

#if defined(__SSE2__)
  for (; pos - 16 >= begin;
       pos -= 16) /// Assuming the pointer cannot overflow. Assuming we can
                  /// compare these pointers.
  {
    __m128i bytes =
...
template <bool positive, ReturnMode return_mode, size_t num_chars, char c01,
          char c02 = 0, char c03 = 0, char c04 = 0, char c05 = 0, char c06 = 0,
          char c07 = 0, char c08 = 0, char c09 = 0, char c10 = 0, char c11 = 0,
          char c12 = 0, char c13 = 0, char c14 = 0, char c15 = 0, char c16 = 0>
inline const char *find_first_symbols_sse42(const char *const begin,
                                            const char *const end) {
  const char *pos = begin;
  ...
  __m128i set = _mm_setr_epi8(c01, c02, c03, c04, c05, c06, c07, c08, c09, c10,
  ...
  for (; pos + 15 < end; pos += 16) {
    ...
    if constexpr (positive) {
      ...
      if (_mm_cmpestrc(set, num_chars, bytes, 16,
                       mode | _SIDD_NEGATIVE_POLARITY))
        return pos + _mm_cmpestri(set, num_chars, bytes, 16,
                                  mode | _SIDD_NEGATIVE_POLARITY);
    }
  }
#endif
  ...
  for (; pos < end; ++pos)
    if ((num_chars == 1 && maybe_negate<positive>(is_in<c01>(*pos))) ||
        ...
        (num_chars == 3 &&
         maybe_negate<positive>(is_in<c01, c02, c03>(*pos))) ||
        (num_chars == 4 &&
         maybe_negate<positive>(is_in<c01, c02, c03, c04>(*pos))) ||
        (num_chars == 5 &&
         maybe_negate<positive>(is_in<c01, c02, c03, c04, c05>(*pos))) ||
        (num_chars == 6 &&
         maybe_negate<positive>(is_in<c01, c02, c03, c04, c05, c06>(*pos))) ||
        (num_chars == 7 &&
         maybe_negate<positive>(
             is_in<c01, c02, c03, c04, c05, c06, c07>(*pos))) ||
        (num_chars == 8 &&
         maybe_negate<positive>(
             is_in<c01, c02, c03, c04, c05, c06, c07, c08>(*pos))) ||
        (num_chars == 9 &&
         maybe_negate<positive>(
             is_in<c01, c02, c03, c04, c05, c06, c07, c08, c09>(*pos))) ||
        (num_chars == 10 &&
         maybe_negate<positive>(
             is_in<c01, c02, c03, c04, c05, c06, c07, c08, c09, c10>(*pos))) ||
        (num_chars == 11 &&
         maybe_negate<positive>(
             is_in<c01, c02, c03, c04, c05, c06, c07, c08, c09, c10, c11>(
                 *pos))) ||
        (num_chars == 12 &&
         maybe_negate<positive>(
             is_in<c01, c02, c03, c04, c05, c06, c07, c08, c09, c10, c11, c12>(
                 *pos))) ||
        (num_chars == 13 &&
         maybe_negate<positive>(is_in<c01, c02, c03, c04, c05, c06, c07, c08,
                                      c09, c10, c11, c12, c13>(*pos))) ||
        (num_chars == 14 &&
         maybe_negate<positive>(is_in<c01, c02, c03, c04, c05, c06, c07, c08,
                                      c09, c10, c11, c12, c13, c14>(*pos))) ||
        (num_chars == 15 &&
         maybe_negate<positive>(
             is_in<c01, c02, c03, c04, c05, c06, c07, c08, c09, c10, c11, c12,
                   c13, c14, c15>(*pos))) ||
...
template <bool positive, ReturnMode return_mode, char... symbols>
inline const char *find_first_symbols_dispatch(const char *begin,
                                               const char *end)
  requires(0 <= sizeof...(symbols) && sizeof...(symbols) <= 16)
{
#if defined(__SSE4_2__)
  if (sizeof...(symbols) >= 5)
    return find_first_symbols_sse42<positive, return_mode, sizeof...(symbols),
                                    symbols...>(begin, end);
#endif
...
inline const char *find_first_symbols_dispatch(const std::string_view haystack,
  ...
  if (symbols.str.size() >= 5)
    return find_first_symbols_sse42<positive, return_mode>(
        haystack.begin(), haystack.end(), symbols);
#endif
...
inline const char *find_first_symbols(std::string_view haystack,
                                      const SearchSymbols &symbols) {
  return detail::find_first_symbols_dispatch<true, detail::ReturnMode::End>(
      haystack, symbols);
}
...
inline char *find_first_not_symbols(char *begin, char *end) {
  return const_cast<char *>(
      detail::find_first_symbols_dispatch<false, detail::ReturnMode::End,
                                          symbols...>(begin, end));
}

inline const char *find_first_not_symbols(std::string_view haystack,
                                          const SearchSymbols &symbols) {
  return detail::find_first_symbols_dispatch<false, detail::ReturnMode::End>(
      haystack, symbols);
}
...
template <char... symbols>
inline const char *find_first_symbols_or_null(const char *begin,
                                              const char *end) {
  return detail::find_first_symbols_dispatch<true, detail::ReturnMode::Nullptr,
                                             symbols...>(begin, end);
}
...
inline char *find_first_symbols_or_null(char *begin, char *end) {
  return const_cast<char *>(
      detail::find_first_symbols_dispatch<true, detail::ReturnMode::Nullptr,
                                          symbols...>(begin, end));
}
...
template <char... symbols>
inline const char *find_first_not_symbols_or_null(const char *begin,
                                                  const char *end) {
  return detail::find_first_symbols_dispatch<false, detail::ReturnMode::Nullptr,
                                             symbols...>(begin, end);
}
...
inline char *find_first_not_symbols_or_null(char *begin, char *end) {
  return const_cast<char *>(
      detail::find_first_symbols_dispatch<false, detail::ReturnMode::Nullptr,
                                          symbols...>(begin, end));
}

inline const char *
find_first_not_symbols_or_null(std::string_view haystack,
                               const SearchSymbols &symbols) {
  return detail::find_first_symbols_dispatch<false,
                                             detail::ReturnMode::Nullptr>(
      haystack, symbols);
}

template <char... symbols>
inline const char *find_last_symbols_or_null(const char *begin,
                                             const char *end) {
...
template <char... symbols>
inline const char *find_last_not_symbols_or_null(const char *begin,
                                                 const char *end) {
...
/// fires a false positive in clang static analyzer. See
/// https://github.com/boostorg/algorithm/issues/63 And
/// https://bugs.llvm.org/show_bug.cgi?id=41141
template <char... symbols, typename To>
inline To &splitInto(To &to, std::string_view what,
