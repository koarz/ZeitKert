...
// more aliases:
// https://mailman.videolan.org/pipermail/x264-devel/2014-May/010660.html
...
#error                                                                         \
    "The only supported platforms are x86_64 and AArch64, PowerPC (work in progress), s390x (work in progress), loongarch64 (experimental) and RISC-V 64 (experimental)"
#endif
...
/// Explicitly allow undefined behaviour for certain functions. Use it as a
/// function attribute. It is useful in case when compiler cannot see (and
/// exploit) it, but UBSan can. Example: multiplication of signed integers with
/// possibility of overflow when both sides are from user input.
...
/// We used to have only ABORT_ON_LOGICAL_ERROR macro, but most of its uses were
/// actually in places where we didn't care about logical errors but wanted to
/// check exactly if the current build type is debug or with sanitizer. This new
/// macro is introduced to fix those places.
...
/// Heavy assertions (that run loops or call complex functions) are allowed in
/// debug builds only. Also it makes sense to call abort() instead of
/// __builtin_unreachable() in debug builds, because SIGABRT is easier to debug
/// than SIGTRAP (the second one makes gdb crazy)
#if !defined(chassert)
#if defined(DEBUG_OR_SANITIZER_BUILD)
// clang-format off
