...
#include "clickhouse/base/defines.h"
#include "clickhouse/base/unit.h"
...
/// Timeout for synchronous request-result protocol call (like Ping or
/// TablesStatus).
 ...
 * Smaller values give better cache locality, less consumption of RAM, but more
 * overhead to process the query.
 */
static constexpr auto DEFAULT_BLOCK_SIZE =
    65409; /// 65536 - PADDING_FOR_SIMD - (PADDING_FOR_SIMD - 1) bytes padding
           /// that we usually have in = arrays

/** Which blocks should be formed for insertion into the table, if we control
 * the formation of blocks. (Sometimes the blocks are inserted exactly such
 * blocks that have been read / transmitted from the outside, and this parameter
 * does not affect their size.) More than DEFAULT_BLOCK_SIZE, because in some
 * tables a block of data on the disk is created for each block (quite a big
 * thing), and if the parts were small, then it would be costly then to combine
 * them.
 */
static constexpr auto DEFAULT_INSERT_BLOCK_SIZE =
    1048449; /// 1048576 - PADDING_FOR_SIMD - (PADDING_FOR_SIMD - 1) bytes
             /// padding that we usually have in arrays
...
static constexpr auto
    DBMS_CONNECTION_POOL_WITH_FAILOVER_DEFAULT_DECREASE_ERROR_PERIOD = 60;
/// replica error max cap, this is to prevent replica from accumulating too many
/// errors and taking too long to recover.
static constexpr auto DBMS_CONNECTION_POOL_WITH_FAILOVER_MAX_ERROR_COUNT = 1000;

/// The boundary on which the blocks for asynchronous file operations should be
/// aligned.
...
/// Actually, there may be multiple acquisitions of different locks for a given
/// table within one query. Check with IStorage class for the list of possible
/// locks
...
#else
/// On Mac OS, MADV_FREE is not lazy, so page_cache_use_madv_free should be
/// disabled. On FreeBSD, it may work but we haven't tested it.
...
static constexpr auto DEFAULT_MMAP_CACHE_MAX_SIZE =
    1_KiB; /// chosen by rolling dice
...
/// -fno-omit-frame-pointer) And they do unwinding frequently (on every
/// malloc/free, thread/mutex operations, etc). They change %rbp during
/// unwinding and it confuses libunwind if signal comes during sanitizer
/// unwinding
///  and query profiler decide to unwind stack with libunwind at this moment.
///
/// Symptoms: you'll get silent Segmentation Fault - without sanitizer message
/// and without usual ClickHouse diagnostics.
///
