...
#define APPLY_FOR_TOKENS(M)                                                    \
  ...
  M(BareWord) /** Either keyword (SELECT) or identifier (column) */            \
                                                                               \
  M(Number) /** Always non-negative. No leading plus. 123 or something like    \
               123.456e12, 0x123p12 */                                         \
  ...
  M(Dot) /** Compound identifiers, like a.b or tuple access operator a.1, (x,  \
            y).2. */                                                           \
  /** Need to be distinguished from floating point number with omitted integer \
   * part: .1 */                                                               \
                                                                               \
  M(Asterisk) /** Could be used as multiplication operator or on it's own:     \
                 "SELECT *" */                                                 \
                                                                               \
  M(HereDoc)                                                                   \
                                                                               \
  ...
  M(Concatenation) /** String concatenation operator: || */                    \
                                                                               \
  M(At)       /** @. Used for specifying user names and also for MySQL-style   \
                 variables. */                                                 \
  M(DoubleAt) /** @@. Used for MySQL-style global variables. */                \
                                                                               \
  /** Order is important. EndOfStream goes after all usual tokens, and special \
   * error tokens goes after EndOfStream. */                                   \
                                                                               \
  M(EndOfStream)                                                               \
                                                                               \
...
private:
  ...
  /// This is needed to disambiguate tuple access operator from floating point
  /// number (.1).
  TokenType prev_significant_token_type =
      TokenType::Whitespace; /// No previous token.
};
